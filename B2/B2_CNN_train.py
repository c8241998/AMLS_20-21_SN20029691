from B2.dataset import CartoonDataset
from B2.models import *
...
def train(model,criterion,optimizer,trainloader,valloader,schedular,epochs,config):
    ...
    for epoch in range(epochs):
        ...
        if val_acc>best_acc_val:
            torch.save(model.state_dict(), 'B2/model.pth')
...
def inference(model,trainloader,valloader):
    ...
    for sample in valloader:
        ...
        for i in range(batch):
            ...
            if(abs(pred_i-label_i)<0.01):
                cnt_val=cnt_val+1.
    train_acc, val_acc = cnt_train/80.,cnt_val/20.
